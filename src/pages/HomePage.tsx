...
export function HomePage() {
  ...
  return (
    <VStack gap={{ base: 6, md: 8 }} align="stretch" w="full">
      <MotionBox
        initial={motionAllowed ? { opacity: 0, y: -20 } : false}
        animate={motionAllowed ? { opacity: 1, y: 0 } : undefined}
        transition={{ duration: motionAllowed ? 0.5 : 0 }}
        bg="#161B22"
        border="1px solid #30363D"
        borderRadius="xl"
        p={{ base: 4, sm: 6, md: 8 }}
      ...
      >
        ...
        <VStack align="start" gap={3} position="relative" zIndex={1}>
          <Heading size={{ base: 'xl', sm: '2xl' }} color="#F0F6FC" fontWeight="800">
            Welcome to DopamiX
          </Heading>
          ...
          >
            <Box w={{ base: 'full', sm: 'auto' }} css={{ '& > button': { width: '100%' } }}>
      ...
      <MotionBox variants={motionAllowed ? container : undefined} initial={motionAllowed ? 'hidden' : false} animate={motionAllowed ? 'show' : undefined}>
        <SimpleGrid columns={{ base: 1, sm: 2, md: 4 }} gap={4}>
          <StatCard icon={<Wallet size={24} />} label="Balance" value={formatCurrency(balance, currencyFormat)} color="#00D395" />
          ...
          <MotionBox variants={motionAllowed ? container : undefined} initial={motionAllowed ? 'hidden' : false} animate={motionAllowed ? 'show' : undefined}>
            <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={4}>
              {featuredEvents.map((event) => (
                <MotionBox key={event.id} variants={motionAllowed ? item : undefined}>
